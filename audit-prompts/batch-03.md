...
pre-filled for Batch 3.

---

```
Run audit Batch 3 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 3 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 3,
if an old manifest from Batch 2 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch02.json, then create the fresh manifest
for Batch 3.
...
Do not advance to Batch 4 on your own. Stop after Batch 3
and wait.
```
