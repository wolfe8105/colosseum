...
pre-filled for Batch 6.

---

```
Run audit Batch 6 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 6 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 6,
if an old manifest from Batch 5 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch05.json, then create the fresh manifest
for Batch 6.
...
Do not advance to Batch 7 on your own. Stop after Batch 6
and wait.
```
