...
2. FIRST-RUN-FILES.md
   The file list. For this run, use Batch B only. Ignore the other
   batches. The file list is under the "## Batch B" heading.
...
audit/manifest.batchPREV.json, then create the fresh manifest for Batch B.
...
Do not advance to the next batch on your own. Stop after Batch B and wait.
```
