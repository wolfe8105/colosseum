...
pre-filled for Batch 2.

---

```
Run audit Batch 2 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 2 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 2,
if an old manifest from Batch 1 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch01.json, then create the fresh manifest
for Batch 2.
...
Do not advance to Batch 3 on your own. Stop after Batch 2
and wait.
```
