...
const ColosseumHome = (() => {
  ...
  // SESSION 7 FIX (Bugs 2-4, 6): Shared escapeHTML helper — used by all render functions
  const _escMap = {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'};
  const esc = (s) => {
    if (typeof ColosseumConfig !== 'undefined' && ColosseumConfig.escapeHTML) return ColosseumConfig.escapeHTML(s);
    ...
    const cards = PLACEHOLDER_LIVE.map(d => `
      <div class="home-live-card">
        <div class="home-live-card-header">
          ${renderLiveBadge()}
          <span class="home-live-round">RD ${d.round}/${d.totalRounds} · ${esc(d.timeLeft)}</span>
        </div>
        <div class="home-matchup">
          <div class="home-debater">
            <span class="home-debater-name">${esc(d.debater1)}</span>
            <span class="home-debater-elo">${d.elo1}</span>
          </div>
          <span class="home-vs">VS</span>
          <div class="home-debater right">
            <span class="home-debater-name">${esc(d.debater2)}</span>
            <span class="home-debater-elo">${d.elo2}</span>
          </div>
        </div>
        <div class="home-live-topic">${esc(d.topic)}</div>
        <div class="home-live-footer">
          <span class="home-spectators">👁 ${d.spectators} watching</span>
          <button class="home-watch-btn" data-section="${esc(d.section)}">WATCH LIVE</button>
        </div>
      </div>
    `).join('');
  ...
  function renderTonightsCard() {
    ...
    const cards = PLACEHOLDER_TONIGHT.map((d, i) => {
      ...
      return `
        <div class="home-tonight-card" style="border-left: 3px solid ${section?.borderColor || 'var(--gold)'};cursor:pointer;" data-prediction-index="${i}">
          <div class="home-tonight-time">${esc(d.time)}</div>
          <div class="home-tonight-matchup">
            <span class="home-tonight-name">${esc(d.debater1)}</span>
            <span class="home-tonight-vs">vs</span>
            <span class="home-tonight-name">${esc(d.debater2)}</span>
          </div>
          <div class="home-tonight-topic">${esc(d.topic)}</div>
          <div class="home-tonight-footer">
            <span class="home-tonight-section">${section?.icon || ''} ${esc(section?.name || '')}</span>
            <button class="home-predict-btn" data-prediction-index="${i}">🎯 PREDICT</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  function openPrediction(index) {
    ...
    // SESSION 7 FIX (Bug 4): All dynamic data escaped via esc()
    modal.innerHTML = `
      <div style="
      ...
      ">
        ...
        <div style="color:#a0a8b8;text-align:center;font-size:12px;margin-bottom:16px;">${esc(d.time)} · ${esc(d.topic)}</div>
        
        <div style="display:flex;gap:8px;margin-bottom:16px;">
          <button class="pred-pick-btn" id="pred-pick-a" style="
          ...
          ">
            <div style="font-family:'Bebas Neue',sans-serif;font-size:18px;letter-spacing:1px;">${esc(d.debater1)}</div>
            <div style="font-size:11px;color:#a0a8b8;">ELO ${d.elo1}</div>
          </button>
          <button class="pred-pick-btn" id="pred-pick-b" style="
            flex:1;padding:16px 8px;background:#1a2d4a;border:2px solid rgba(255,255,255,0.1);
            border-radius:12px;text-align:center;cursor:pointer;color:#f0f0f0;
          ">
            <div style="font-family:'Bebas Neue',sans-serif;font-size:18px;letter-spacing:1px;">${esc(d.debater2)}</div>
        ...
        <div style="margin-bottom:16px;">
          ...
          <div style="display:flex;gap:6px;">
            <button class="pred-wager-btn" data-wager="5" style="padding:8px 14px;background:#1a2d4a;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#d4a843;font-weight:700;font-size:13px;cursor:pointer;">5</button>
            <button class="pred-wager-btn" data-wager="10" style="padding:8px 14px;background:#1a2d4a;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#d4a843;font-weight:700;font-size:13px;cursor:pointer;">10</button>
            <button class="pred-wager-btn" data-wager="25" style="padding:8px 14px;background:#1a2d4a;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#d4a843;font-weight:700;font-size:13px;cursor:pointer;">25</button>
        ...
        <div style="display:flex;gap:8px;">
          <button id="pred-cancel-btn" style="
          ...
          ">CANCEL</button>
          <button id="pred-submit-btn" style="
    ...
    // Wire buttons via addEventListener instead of inline onclick
    document.getElementById('pred-pick-a')?.addEventListener('click', () => {
      document.getElementById('pred-pick-a')?.classList.add('selected');
      document.getElementById('pred-pick-b')?.classList.remove('selected');
    });
    document.getElementById('pred-pick-b')?.addEventListener('click', () => {
      document.getElementById('pred-pick-b')?.classList.add('selected');
      document.getElementById('pred-pick-a')?.classList.remove('selected');
    });
    modal.querySelectorAll('.pred-wager-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const inp = document.getElementById('pred-amount');
        if (inp) inp.value = btn.dataset.wager;
      });
    });
    document.getElementById('pred-cancel-btn')?.addEventListener('click', () => {
      document.getElementById('prediction-modal')?.remove();
    });
    document.getElementById('pred-submit-btn')?.addEventListener('click', () => {
      _submitPrediction(index);
    });
  ...
  function renderSectionContent(section) {
    ...
    if (liveDeb.length > 0) {
      liveHtml = liveDeb.map(d => `
        <div class="home-section-live-mini">
          ...
          <span class="home-mini-matchup">${esc(d.debater1)} vs ${esc(d.debater2)}</span>
          <button class="home-mini-watch" data-section="${esc(section.id)}">WATCH</button>
        </div>
      `).join('');
    }

    // SESSION 7 FIX (Bug 1): Use shared esc() + data attributes instead of inline onclick with raw text
    const takesHtml = takes.map(t => {
      ...
      <div class="home-take-card">
        <div class="home-take-header">
          <span class="home-take-user">${safeUser}</span>
          <span class="home-take-elo">⚔ ${t.elo}</span>
          <span class="home-take-time">${esc(t.time)}</span>
        </div>
        <div class="home-take-body">${safeText}</div>
        <div class="home-take-actions">
          <button class="home-take-btn fire" data-action="react" data-section="${esc(section.id)}">🔥 ${t.reactions}</button>
          <button class="home-take-btn challenge" data-action="challenge" data-section="${esc(section.id)}">⚔️ ${t.challenges}</button>
          <button class="home-take-btn" data-action="voicememo" data-user="${safeUser}" data-text="${safeText}" data-section="${esc(section.id)}" style="color:var(--white-dim);">🎤</button>
          <button class="home-take-bet" data-action="bet" data-section="${esc(section.id)}">BET.</button>
        </div>
      </div>
    `;
    }).join('');
    ...
    return `
      <div class="home-section-content" id="section-content-${section.id}" style="display:none;">
        ${liveHtml}
        ${takesHtml}
        <button class="home-section-more" data-action="loadmore" data-section="${esc(section.id)}">Load more from ${esc(section.name)}</button>
      </div>
    `;
  }
    ...
    return `
      <div class="home-composer">
        <div class="home-composer-row" style="display:flex;gap:8px;align-items:center;">
          <div class="home-composer-prompt" id="home-composer-prompt" data-action="open-composer" style="flex:1;">
            🔥 Drop a hot take...
          </div>
          <button class="home-mic-btn" data-action="record-take" title="Voice Take" style="
    ...
    return `
      <div class="home-activity-bar">
        <div class="home-activity-header">
          ...
          <button class="home-activity-see-all" data-action="see-all-activity">See all</button>
        </div>
        <div class="home-activity-scroll">
          ${items.map(i => `
            <div class="home-activity-item">
              <span class="home-activity-icon">${esc(i.icon)}</span>
              <span class="home-activity-text">${esc(i.text)}</span>
              <span class="home-activity-time">${esc(i.time)}</span>
            </div>
          `).join('')}
        </div>
      </div>
  ...
  function render(containerId) {
    ...
    // SESSION 7 FIX: Event delegation for all data-attribute buttons (replaces inline onclick)
    container.addEventListener('click', (e) => {
      const btn = e.target.closest('[data-action]');
      if (!btn) {
        // Handle watch buttons and prediction cards (no data-action)
        const watchBtn = e.target.closest('.home-watch-btn, .home-mini-watch');
        if (watchBtn) { watchDebate(watchBtn.dataset.section); return; }
        const predCard = e.target.closest('.home-tonight-card[data-prediction-index]');
        if (predCard && !e.target.closest('.home-predict-btn')) { openPrediction(parseInt(predCard.dataset.predictionIndex)); return; }
        return;
      }
      const action = btn.dataset.action;
      const section = btn.dataset.section;
      switch (action) {
        case 'react': react(section); break;
        case 'challenge': challenge(section); break;
        case 'bet': bet(section); break;
        case 'loadmore': loadMore(section); break;
        case 'voicememo':
          if (typeof ColosseumVoiceMemo !== 'undefined' && ColosseumVoiceMemo.replyToTake) {
            ColosseumVoiceMemo.replyToTake('', btn.dataset.user || '', btn.dataset.text || '', section);
          }
          break;
        case 'open-composer': openComposer(); break;
        case 'record-take':
          if (typeof ColosseumVoiceMemo !== 'undefined' && ColosseumVoiceMemo.recordTake) {
            ColosseumVoiceMemo.recordTake('trending');
          }
          break;
        case 'see-all-activity': document.getElementById('notif-btn')?.click(); break;
      }
    });

    // Handle prediction button clicks (need stopPropagation equivalent)
    container.querySelectorAll('.home-predict-btn[data-prediction-index]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        openPrediction(parseInt(btn.dataset.predictionIndex));
      });
    });
  ...
  function bet(section) {
    if (typeof ColosseumAuth !== 'undefined' && !ColosseumAuth.requireAuth('place a bet')) return;
    showToast('💰 BET. — Challenge created!');
  }
  ...
  // SESSION 64 FIX: Use global showToast from ColosseumConfig instead of duplicate
  // SESSION 7 FIX (Bug 5): Added console.log fallback so actions aren't silently swallowed
...
// SESSION 64 FIX: Removed auto-render.
// The spoke carousel (built in index.html appInit) IS the home screen.
// ColosseumHome.render() was nuking the carousel and all its touch listeners
// by replacing innerHTML 900ms after load. Module stays available if needed
// but does NOT auto-fire.
