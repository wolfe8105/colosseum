...
window.ColosseumArena = (() => {
    ...
    try {
      ...
      if (error || !data || data.length === 0) {
        ...
        const { data: autoData } = await sb.from('auto_debates')
          .select('id, topic, side_a_name, side_b_name, final_score_a, final_score_b, status, created_at')
          .eq('status', 'active')
  ...
  function renderAutoDebateCard(d) {
    ...
    return `<div class="arena-card" onclick="window.location.href='colosseum-auto-debate.html?id=${d.id}'">
      ...
      <div class="arena-card-vs">
        <span>${sanitize(d.side_a_name)}</span>
        <span class="vs">VS</span>
        <span>${sanitize(d.side_b_name)}</span>
        <span class="arena-card-score">${d.final_score_a}–${d.final_score_b}</span>
      </div>
  ...
  async function handleAIResponse(debate, userText) {
    ...
    // TODO: Replace with actual Groq Edge Function call
  ...
  async function generateAIDebateResponse(topic, userArg, round, totalRounds) {
    ...
    try {
      const supabaseUrl = (typeof ColosseumConfig !== 'undefined' && ColosseumConfig.supabaseUrl)
        ? ColosseumConfig.supabaseUrl
        : null;
