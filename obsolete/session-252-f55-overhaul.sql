...
-- A.2 — Drop old arsenal_references (CASCADE kills FKs in debate_reference_loadouts, reference_verifications)
...
-- A.10 — Reference sockets (F-57 prep)
CREATE TABLE public.reference_sockets (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  reference_id    UUID NOT NULL REFERENCES public.arsenal_references(id) ON DELETE CASCADE,
  socket_index    INTEGER NOT NULL,
  modifier_id     UUID NOT NULL,  -- FK to F-57 user_modifiers added in F-57 Phase 1 build
...
CREATE POLICY "Reference sockets readable by all" ON public.reference_sockets
  FOR SELECT USING (true);

CREATE POLICY "Reference sockets inserted by server" ON public.reference_sockets
  FOR INSERT WITH CHECK (false);

CREATE POLICY "Reference sockets deleted by server" ON public.reference_sockets
  FOR DELETE USING (false);
...
-- ############################################################
-- PHASE 6: RETIRE DEAD LEGACY RPCs
-- ############################################################
