 ...
 * Session 195 (L1): power-up costs loaded from app_config table via app-config.ts.
 */

import { safeRpc } from './auth.ts';
import { escapeHTML } from './config.ts';
import { getTier, getPowerUpSlots, getNextTier } from './tiers.ts';
import { getPowerUpCost } from './app-config.ts';
...
// ============================================================
// STATIC CATALOG
// cost values here are display fallbacks only.
// Live values come from app_config via getPowerUpCost().
// ============================================================
...
// ============================================================
// RENDER: SHOP
// Fetches live costs from app_config. Falls back to CATALOG if unavailable.
// ============================================================

export async function renderShop(tokenBalance: number): Promise<string> {
  const balance = tokenBalance || 0;

  const items = await Promise.all(
    (Object.entries(CATALOG) as [PowerUpId, PowerUpCatalogEntry][]).map(async ([id, pu]) => {
      const liveCost = await getPowerUpCost(id);
      const cost = liveCost > 0 ? liveCost : pu.cost;
      ...
      return `
        <div class="powerup-shop-item" style="display:flex;align-items:center;gap:12px;padding:12px;background:#1a1a2e;border:1px solid #2a2a3e;border-radius:8px;margin-bottom:8px;">
          ...
          <button class="powerup-buy-btn" data-id="${id}" data-cost="${cost}" ${canAfford ? '' : 'disabled'}
            style="padding:8px 14px;border:none;border-radius:6px;background:${canAfford ? 'linear-gradient(135deg,var(--mod-text-heading),#B8860B)' : '#2a2a3e'};color:${canAfford ? '#0f0f1a' : '#666'};font-family:var(--mod-font-ui);font-size:13px;font-weight:600;cursor:${canAfford ? 'pointer' : 'default'};white-space:nowrap;">${cost} 🪙</button>
        </div>`;
    })
  );
  ...
  document.querySelectorAll('.powerup-slot.empty').forEach(slot => {
    slot.addEventListener('click', () => {
      ...
      document.querySelectorAll('.powerup-slot').forEach(s => {
        (s as HTMLElement).style.borderColor = s === slot
          ? 'var(--mod-text-heading)'
          : (s.classList.contains('filled') ? 'var(--mod-text-heading)44' : '#2a2a3e');
      });
    });
  });
...
export function renderActivationBar(equipped: EquippedItem[]): string {
  ...
  const buttons = equipped.map(eq => {
    ...
    return `
      <button class="powerup-activate-btn ${isPassive ? 'passive' : ''}" data-id="${eq.power_up_id}" data-slot="${eq.slot_number}"
