...
### Last Updated: Session 61 (March 8, 2026) — Public profile pages: /u/username, serverless OG tags, Google-indexable
...
4.50. **Bot army expanding.** Bluesky (done). Lemmy (done, L2+L3 live Session 59). Mastodon (P3). Twitter free tier useless.

## 4.51. OPEN DECISIONS
4.51.1. Launch date: what's real?
4.51.2. Domain: thecolosseum.app or keep current URLs?
4.51.3. ✅ **Public profile pages — BUILT.** `/u/username` via Vercel serverless function. Dynamic OG tags for crawlers. Google-indexable. Session 61.
...
| File | Purpose |
|------|---------|
| api/profile.js | Public profile page — fetches profiles_public, returns full HTML with dynamic OG tags. Route: /u/:username. Session 61. |
...
10.8. Platforms: Bluesky (done, L2 posting), Lemmy (done, L2+L3 posting), Reddit (pending API), Discord (deferred).
...
## 11.10. Session 57 (Mar 8) — Lemmy Bot Wired and Live

11.10.1. ✅ **Lemmy bot files built** — `leg2-lemmy-poster.js` (Leg 2 proactive hot take poster) and `leg3-lemmy-poster.js` (Leg 3 auto-debate rage-click poster). Raw fetch LemmyClient class, JWT persisted per session, max 2 posts/community/day. 10 communities across lemmy.ml, lemmy.world, lemmy.blahaj.zone.
11.10.2. ✅ **bot-config.js updated** — lemmy block added (instance, username, password, maxPostsPerCommunityPerDay=2, maxPostsPerDay=4). Flags: `leg2Lemmy`, `leg3LemmyPost`. Both flags read from `.env`.
11.10.3. ✅ **bot-engine.js patched** — requires for both Lemmy posters added. Pipeline calls added inside flag guards for both Leg 2 and Leg 3 loops.
11.10.4. ✅ **VPS .env updated** — LEMMY_INSTANCE, LEMMY_USERNAME, LEMMY_PASSWORD set. LEG2_LEMMY_ENABLED=false, LEG3_LEMMY_POST_ENABLED=false (dormant until account ready).
11.10.5. ✅ **Bot army healthy** — Full `pm2 delete` + `pm2 start` cleared module cache. Status: online, 0 restarts, 95mb. Root cause of prior crash: `cp` aliased to `cp -i` silently failed to overwrite. Fix: `\cp` bypasses alias.
11.10.6. ⚠️ **Lemmy flags OFF** — Bot will not post to Lemmy until account is marked as bot on lemmy.world and bio is set. Then flip LEG2_LEMMY_ENABLED=true + pm2 restart.

## 11.11. Session 58 (Mar 8) — Lemmy Activation + Bot Config Fixes

11.11.1. ✅ **Lemmy poster files moved to correct directory** — leg2-lemmy-poster.js and leg3-lemmy-poster.js were in /opt/colosseum/bot-army/ (wrong). Moved to /opt/colosseum/bot-army/colosseum-bot-army/ (correct). LM-148.
11.11.2. ✅ **ecosystem.config.js rewritten as full file** — Lemmy and Bluesky env vars now correctly inside the apps env: block. LM-146 + LM-147. Vars: LEMMY_INSTANCE, LEMMY_USERNAME, LEMMY_PASSWORD, LEG2_LEMMY_ENABLED=true, LEG3_LEMMY_POST_ENABLED=true, LEG2_BLUESKY_ENABLED=true, LEG3_BLUESKY_POST_ENABLED=false.
11.11.3. ✅ **leg2Bluesky + leg3BlueskyPost added to bot-config.js flags block** — were missing entirely, causing Bluesky to vanish from Features line. LM-149.
11.11.4. ✅ **Bluesky restored to Features line** — confirmed: Features: L1-Discord, L2-News, L2-Debates, L3-AutoDebate, L2-Bluesky.
11.11.5. ✅ **Lemmy now in Features line** — Fixed Session 59. formatFlags() entries added. Lemmy un-nested from Bluesky conditionals.
11.11.6. ⚠️ **Groq TPD cap hitting daily** — 100k token/day free tier exhausted by end of day. Bot falls back to templates. Resets midnight UTC.
...
## 11.12. Session 59 (Mar 8) — Three Critical Bot Fixes

11.12.1. ✅ **CRITICAL: Dead bot URLs fixed** — Every bot link since go-live was broken. `debateLandingPath: null` in bot-config.js caused URL concatenation to produce `https://colosseum-six.vercel.appnull?id=...`. Fixed via `sed -i` to set `debateLandingPath: '/colosseum-debate-landing.html'`. LM-150.
11.12.2. ✅ **Lemmy poster files delivered to VPS** — leg2-lemmy-poster.js and leg3-lemmy-poster.js were never on VPS despite multiple sessions claiming they were. Transferred via `scp` from Windows to correct directory. LM-148 confirmed never actually fixed until now.
11.12.3. ✅ **Lemmy un-nested from Bluesky conditionals** — In bot-engine.js, Lemmy calls were inside `if (config.flags.leg2Bluesky)` and `if (config.flags.leg3BlueskyPost)` blocks. Lemmy would never post unless Bluesky was also enabled. Fixed: independent conditionals for each platform.
11.12.4. ✅ **formatFlags() Lemmy entries added** — `L2-Lemmy` and `L3-Lemmy` now appear in Features line. NT 12.2.7 complete.
11.12.5. ✅ **.env file exists (LM-147 was wrong)** — `.env` file at `/opt/colosseum/bot-army/colosseum-bot-army/.env` is the primary source of env vars. `require('dotenv').config()` loads it before PM2 ecosystem vars. Updated Lemmy flags to `true` in .env. LM-151.
11.12.6. ✅ **Bot army healthy** — Features: L1-Discord, L2-News, L2-Debates, L3-AutoDebate, L2-Bluesky, L2-Lemmy, L3-Lemmy. 0 restarts. 18.5mb.
11.12.7. ⚠️ **Groq TPD cap still hitting daily** — 100k token/day free tier. Falls back to templates. Known, not urgent.
11.12.8. ⚠️ **Windows download cache caused stale scp** — First scp of bot-engine.js uploaded a cached old file. Browser served stale download. Fixed by re-outputting with unique filename. LM-152.

## 11.13. Session 60 (Mar 8) — Area 3 Polish: Elo Explainer, Friendly Errors, Shimmer, Toast

11.13.1. ✅ **Global showToast() added to colosseum-config.js** — Replaces 4+ scattered inline toast creators across modules. Types: success (gold), error (red), info (neutral). Branded with Barlow Condensed, 2.5s/4s duration, auto-removes.
11.13.2. ✅ **friendlyError() added to colosseum-config.js** — Centralized error translator. Maps rate limits, JWT expiry, network failures, duplicates, 5xx, auth, mic, Stripe errors to human-friendly copy. Fallback: "Something went sideways. Give it another shot."
11.13.3. ✅ **Elo explainer modal built** — Gold "?" icon next to ELO tab in leaderboard. Taps open bottom-sheet with: what Elo is, how it moves (big gain/small gain/big drop/small drop), what the numbers mean (1000–1800+), and note that Casual keeps rating safe.
11.13.4. ✅ **Shimmer loading skeletons** — `.colo-shimmer` CSS class in index.html. Dark-on-dark shimmer matching navy palette. Leaderboard shows 6 shimmer rows instead of "Loading rankings..." text.
11.13.5. ✅ **Toast/modal animations** — `@keyframes coloToastIn`, `coloFadeIn`, `coloSlideUp` added to index.html. Elo modal slides up, toast slides down.
11.13.6. ✅ **Arena errors wired to friendlyError()** — 6 user-facing error spots patched: queue join, reference submission, moderator ruling (allow + deny), moderator score (debater + spectator). All fall back to raw error if friendlyError unavailable.
11.13.7. ✅ **Payment toasts migrated** — index.html payment success/cancel toasts now use global showToast() instead of inline createElement.
11.13.8. ✅ **Celebrity event system moved to Old Testament** — remains a future idea, not a current build priority.

## 11.14. Session 61 (Mar 8) — Public Profile Pages

11.14.1. ✅ **Public profile pages built** — `/u/username` route via Vercel serverless function (`api/profile.js`). Queries `profiles_public` view via Supabase REST API (anon key, `USING(true)` RLS). Returns full HTML with dynamic OG tags server-side — crawlers see real user data (name, Elo, record, bio).
11.14.2. ✅ **OG tags server-rendered** — `og:title` = "displayName — Elo XXXX | The Colosseum", `og:description` = bio + W/L/D record, `og:type` = "profile". Twitter Card included. Social sharing on Bluesky/Discord/Lemmy will show real profile data.
11.14.3. ✅ **Google-indexable** — Profile pages have `<link rel="canonical">`, no `X-Robots-Tag: noindex` (that header only matches `/api/*` source pattern, not `/u/*`). Google can crawl and index all profiles.
11.14.4. ✅ **vercel.json updated** — Rewrite added: `/u/:username` → `/api/profile?username=:username`. First entry in rewrites array.
11.14.5. ✅ **Design DNA matched** — Navy diagonal gradient, frosted glass cards, Cinzel + Barlow Condensed, gold rank badges, mobile-first. Includes: avatar (initials fallback), rank tier badge (Rookie/Gladiator/Contender/Champion/Legendary), Elo display, W/L/D record bar, stat grid (debates/win rate/level), streaks, member since, CTA buttons (Challenge/Follow).
11.14.6. ✅ **404 page themed** — "GLADIATOR NOT FOUND" page for invalid/missing usernames. Same design DNA.
11.14.7. ✅ **5-minute edge cache** — `s-maxage=300, stale-while-revalidate=600` on successful profile responses. Reduces Supabase load for popular profiles.
11.14.8. ✅ **Username validation** — Server-side regex `^[a-zA-Z0-9_]{3,20}$` matches existing DB constraint. Invalid usernames get 404 without hitting Supabase.
11.14.9. ✅ **Avatar URL sanitization** — `sanitizeAvatarUrl()` whitelists only `https://` protocol. Blocks `javascript:`, `data:text/html`, and other dangerous URI schemes. LM-158.
11.14.10. ⚠️ **No dynamic OG image yet** — Uses `og-card-default.png` for all profiles. Could add per-user OG images via Vercel OG (@vercel/og) later.
11.14.11. ⚠️ **Vercel env vars recommended** — Function uses hardcoded fallback for SUPABASE_URL and SUPABASE_ANON_KEY. Cleaner to set these in Vercel Dashboard → Settings → Environment Variables.
...
12.1.9. ✅ **Lemmy account setup DONE** — bot account marked, bio set, flags enabled, posting live. Session 59.
...
12.2.2. 📦 **Celebrity debate event system** — moved to Old Testament for future reference. Not a current build priority.
12.2.3. ⏳ **Area 2 remaining** — Voice memo UX + keyboard handling (needs real device test).
12.2.4. ✅ **Area 3 COMPLETE** — Elo explainer (leaderboard bottom-sheet), friendlyError() centralized error map, showToast() global toast, shimmer loading skeletons, arena errors wired. Session 60.
...
12.2.6. ✅ **Lemmy bot files built and wired** — leg2-lemmy-poster.js, leg3-lemmy-poster.js, bot-config.js updated, bot-engine.js patched. Flags off. Flip when account ready.
12.2.7. ✅ **Lemmy formatFlags fix DONE** — L2-Lemmy and L3-Lemmy entries added to formatFlags() in bot-engine.js. Lemmy un-nested from Bluesky conditionals. Session 59.
12.2.8. Watch what happens — monitor via bot_stats_24h + auto_debate_stats + event_log + `pm2 logs`
12.2.9. ✅ **Public profile pages BUILT** — `/u/username` serverless function, dynamic OG tags, Google-indexable, themed 404. Session 61.
...
12.3.7. ⏳ bot-engine.js not in repo — VPS-only file, source of truth on VPS. Backup: bot-engine.js.bak (pre-Session 59 version).

## 12.4. REMAINING BOT ARMY TASKS
12.4.1. ⏳ Reddit API approval → update .env → enable Reddit legs
12.4.2. ✅ Lemmy bot built, wired, files on VPS, flags on, posting live. Session 59.
