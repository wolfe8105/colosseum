...
4.67. **Browser back button wired for arena.** pushState on real screen changes + overlay opens. popstate closes overlays or returns to lobby. _skipNextPop flag prevents chain reactions. Pattern: push on open, history.back() on user-close. (Session 44)
...
12.2.18. ⏳ **Security audit Session A** — Stripe webhook rewrite (idempotency + raw body + Deno.serve()). innerHTML audit → DOMPurify if needed. See SECURITY-AUDIT-BATTLE-PLAN.md.
12.2.19. ⏳ **Security audit Session B** — Supabase Performance + Security Advisor. RLS indexes. TO authenticated scoping. SECURITY DEFINER function audit.
12.2.20. ⏳ **Security audit Session C** — Rate limit race condition (FOR UPDATE). CSP header audit.
12.2.21. ⏳ **Security audit Session D** — Bot army: jitter on cron intervals, human-like delays, account warmup.
12.2.22. ⏳ **Security audit Session E** — Auth: TOKEN_REFRESHED handling, graceful 401 recovery.
...
11.23.6. ⏳ **Next session: Start Session A of battle plan** — Stripe webhook rewrite (raw body + idempotency table + Deno.serve() pattern) + innerHTML audit for DOMPurify need.
