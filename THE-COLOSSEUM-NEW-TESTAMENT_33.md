...
### Last Updated: Session 103 (March 14, 2026)
...
| colosseum-debate-landing.html | Ungated landing, vote without signup, OG tags, backend vote persistence via landing_vote_counts table |
...
## Database: 25 SQL migrations, 35 tables, 44+ server functions
...
- ⏳ **Unwired edges needing SQL:** E131 (share live debate link), E144/E145/E149 (`check_queue_status` RPC needs `opponent_id`), E211 (group hot take posting UI), E212/E215 (GvG unbuilt)
...
- **Landing page vote persistence (Session 103)** — `landing_vote_counts` table with RLS (no direct access). Two SECURITY DEFINER RPCs: `cast_landing_vote(p_topic, p_side)` and `get_landing_votes(p_topics)`. Both granted to `anon` role. `colosseum-debate-landing.html` uses its own lightweight Supabase client (no auth session, no token refresh). `voteCounted` flag prevents double-counting between optimistic render and backend response. Seed data carries forward hardcoded demo counts.
...
**Remaining docs (13 files):** NT, OT, LM, CLAUDE.md, War Chest, War Plan, Product Vision, Ideas Master Map, Bot Deployment Guide, Deployment Guide, Setup Guide, Telegram Bot Setup, Discord Bot Setup, Setup Bots
