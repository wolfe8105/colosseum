# GATEKEEPER RUN — F-27 Reference Library Browse
...
Feature: F-27 — Reference Library Browse
Source file: src/reference-arsenal.armory.ts
Spec: First check docs/product/ for a dedicated spec file for F-27. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-27.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/reference-arsenal.armory.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/reference-arsenal.armory.ts
...
Assert every import in src/reference-arsenal.armory.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-reference-arsenal-armory.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-reference-arsenal-armory.test.ts
git commit -m "test(gk): F-27 reference armory — gk tests"
git push
