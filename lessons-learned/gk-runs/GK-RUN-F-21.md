# GATEKEEPER RUN — F-21 ELO Scoring
...
Feature: F-21 — ELO Scoring
Source file: src/scoring.ts
Spec: First check docs/product/ for a dedicated spec file for F-21. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-21.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/scoring.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/scoring.ts
...
Assert every import in src/scoring.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-scoring.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-scoring.test.ts
git commit -m "test(gk): F-21 ELO scoring — gk tests"
git push
