# GATEKEEPER RUN — F-18 Group Auditions
...
Feature: F-18 — Group Auditions
Source file: src/pages/groups.auditions.ts
Spec: First check docs/product/ for a dedicated spec file for F-18. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-18.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/groups.auditions.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/groups.auditions.ts
...
Assert every import in src/pages/groups.auditions.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-groups-auditions.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-groups-auditions.test.ts
git commit -m "test(gk): F-18 group auditions — gk tests"
git push
