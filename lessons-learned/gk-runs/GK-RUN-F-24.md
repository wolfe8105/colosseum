...
Feature: F-24 — Search
Source file: src/search.ts
Spec: First check docs/product/ for a dedicated spec file for F-24. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-24.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/search.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/search.ts
...
Assert every import in src/search.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-search.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-search.test.ts
git commit -m "test(gk): F-24 search — gk tests"
git push
