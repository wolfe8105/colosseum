# GATEKEEPER RUN — F-23 DM Chat System
...
Feature: F-23 — DM Chat System
Source file: src/dm/dm.ts
Spec: First check docs/product/ for a dedicated spec file for F-23. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-23.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/dm/dm.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/dm/dm.ts
...
Assert every import in src/dm/dm.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-dm.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-dm.test.ts
git commit -m "test(gk): F-23 DM chat — gk tests"
git push
