# GATEKEEPER RUN — F-36 Onboarding Drip
...
Feature: F-36 — Onboarding Drip
Source file: src/onboarding-drip.ts
Spec: First check docs/product/ for a dedicated spec file for F-36. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-36.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/onboarding-drip.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/onboarding-drip.ts
...
Assert every import in src/onboarding-drip.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-onboarding-drip.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-onboarding-drip.test.ts
git commit -m "test(gk): F-36 onboarding drip — gk tests"
git push
