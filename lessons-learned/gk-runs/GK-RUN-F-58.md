# GATEKEEPER RUN — F-58 Sentiment Tipping
...
Feature: F-58 — Sentiment Tipping
Source file: src/staking.rpc.ts
Spec: First check docs/product/ for a dedicated spec file for F-58. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-58.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/staking.rpc.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/staking.rpc.ts
...
Assert every import in src/staking.rpc.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-staking-rpc.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-staking-rpc.test.ts
git commit -m "test(gk): F-58 sentiment tipping — gk tests"
git push
