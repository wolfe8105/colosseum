# GATEKEEPER RUN — F-02 Match Found Accept Decline
...
Feature: F-02 — Match Found Accept Decline
Source file: src/arena/arena-match-found.ts
Spec: First check docs/product/ for a dedicated spec file for F-02. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-02.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-match-found.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-match-found.ts
...
Assert every import in src/arena/arena-match-found.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-match-found.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-match-found.test.ts
git commit -m "test(gk): F-02 match found — gk tests"
git push
