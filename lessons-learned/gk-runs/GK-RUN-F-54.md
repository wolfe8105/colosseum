# GATEKEEPER RUN — F-54 Private Profile Toggle
...
Feature: F-54 — Private Profile Toggle
Source file: src/auth.profile.ts
Spec: First check docs/product/ for a dedicated spec file for F-54. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-54.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/auth.profile.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/auth.profile.ts
...
Assert every import in src/auth.profile.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-auth-profile.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-auth-profile.test.ts
git commit -m "test(gk): F-54 private profile toggle — gk tests"
git push
