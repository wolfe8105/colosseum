# GATEKEEPER RUN — F-09 Token Prediction Staking
...
Feature: F-09 — Token Prediction Staking
Source file: src/staking.ts
Spec: First check docs/product/ for a dedicated spec file for F-09. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-09.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/staking.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/staking.ts
...
Assert every import in src/staking.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-staking.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-staking.test.ts
git commit -m "test(gk): F-09 token staking — gk tests"
git push
