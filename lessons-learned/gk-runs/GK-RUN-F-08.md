# GATEKEEPER RUN — F-08 Tournament System
...
Feature: F-08 — Tournament System
Source file: src/tournaments.ts
Spec: First check docs/product/ for a dedicated spec file for F-08. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-08.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/tournaments.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/tournaments.ts
...
Assert every import in src/tournaments.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-tournaments.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-tournaments.test.ts
git commit -m "test(gk): F-08 tournament system — gk tests"
git push
