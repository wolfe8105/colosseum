# GATEKEEPER RUN — F-19 Share and Referral
...
Feature: F-19 — Share and Referral
Source file: src/share.ts
Spec: First check docs/product/ for a dedicated spec file for F-19. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-19.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/share.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/share.ts
...
Assert every import in src/share.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-share.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-share.test.ts
git commit -m "test(gk): F-19 share referral — gk tests"
git push
