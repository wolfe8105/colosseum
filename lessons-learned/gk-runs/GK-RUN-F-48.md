# GATEKEEPER RUN — F-48 Mod-Initiated Debate
...
Feature: F-48 — Mod-Initiated Debate
Source file: src/arena/arena-mod-debate-picker.ts
Spec: First check docs/product/ for a dedicated spec file for F-48. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-48.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-mod-debate-picker.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-mod-debate-picker.ts
...
Assert every import in src/arena/arena-mod-debate-picker.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-mod-debate-picker.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-mod-debate-picker.test.ts
git commit -m "test(gk): F-48 mod debate picker — gk tests"
git push
