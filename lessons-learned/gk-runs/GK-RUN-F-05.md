# GATEKEEPER RUN — F-05 Debate Recording and Replay
...
Feature: F-05 — Debate Recording and Replay
Source file: src/pages/spectate.ts
Spec: First check docs/product/ for a dedicated spec file for F-05. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-05.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/spectate.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/spectate.ts
...
Assert every import in src/pages/spectate.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-spectate.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-spectate.test.ts
git commit -m "test(gk): F-05 debate replay — gk tests"
git push
