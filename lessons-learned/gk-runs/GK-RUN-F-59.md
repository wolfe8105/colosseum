# GATEKEEPER RUN — F-59 Invite Rewards
...
Feature: F-59 — Invite Rewards
Source file: src/pages/home.invite.ts
Spec: First check docs/product/ for a dedicated spec file for F-59. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-59.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/home.invite.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/home.invite.ts
...
Assert every import in src/pages/home.invite.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-home-invite.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-home-invite.test.ts
git commit -m "test(gk): F-59 invite rewards — gk tests"
git push
