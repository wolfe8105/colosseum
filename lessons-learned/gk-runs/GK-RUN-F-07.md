# GATEKEEPER RUN — F-07 Spectator Features
...
Feature: F-07 — Spectator Features
Source file: src/arena/arena-feed-spec-chat.ts
Spec: First check docs/product/ for a dedicated spec file for F-07. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-07.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-feed-spec-chat.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-feed-spec-chat.ts
...
Assert every import in src/arena/arena-feed-spec-chat.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-feed-spec-chat.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-feed-spec-chat.test.ts
git commit -m "test(gk): F-07 spectator chat — gk tests"
git push
