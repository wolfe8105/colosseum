# GATEKEEPER RUN — F-28 Bounty Board
...
Feature: F-28 — Bounty Board
Source file: src/bounties.ts
Spec: First check docs/product/ for a dedicated spec file for F-28. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-28.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/bounties.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/bounties.ts
...
Assert every import in src/bounties.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-bounties.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-bounties.test.ts
git commit -m "test(gk): F-28 bounty board — gk tests"
git push
