# GATEKEEPER RUN — F-15 Token Earn System
...
Feature: F-15 — Token Earn System
Source file: src/tokens.ts
Spec: First check docs/product/ for a dedicated spec file for F-15. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-15.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/tokens.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/tokens.ts
...
Assert every import in src/tokens.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-tokens.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-tokens.test.ts
git commit -m "test(gk): F-15 token earn — gk tests"
git push
