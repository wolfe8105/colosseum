# GATEKEEPER RUN — F-53 Profile Debate Archive
...
Feature: F-53 — Profile Debate Archive
Source file: src/profile-debate-archive.ts
Spec: First check docs/product/ for a dedicated spec file for F-53. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-53.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/profile-debate-archive.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/profile-debate-archive.ts
...
Assert every import in src/profile-debate-archive.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-profile-debate-archive.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-profile-debate-archive.test.ts
git commit -m "test(gk): F-53 profile debate archive — gk tests"
git push
