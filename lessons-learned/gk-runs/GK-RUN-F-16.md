# GATEKEEPER RUN — F-16 Group Settings
...
Feature: F-16 — Group Settings
Source file: src/pages/groups.settings.ts
Spec: First check docs/product/ for a dedicated spec file for F-16. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-16.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/groups.settings.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/groups.settings.ts
...
Assert every import in src/pages/groups.settings.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-groups-settings.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-groups-settings.test.ts
git commit -m "test(gk): F-16 group settings — gk tests"
git push
