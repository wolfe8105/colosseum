# GATEKEEPER RUN — F-01 Queue Waiting Room
...
Feature: F-01 — Queue Waiting Room
Source file: src/arena/arena-queue.ts
Spec: First check docs/product/ for a dedicated spec file for F-01. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-01.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-queue.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-queue.ts
...
Assert every import in src/arena/arena-queue.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-queue.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-queue.test.ts
git commit -m "test(gk): F-01 queue waiting room — gk tests"
git push
