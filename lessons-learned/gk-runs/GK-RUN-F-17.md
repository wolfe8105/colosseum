# GATEKEEPER RUN — F-17 Group Entry Requirements
...
Feature: F-17 — Group Entry Requirements
Source file: src/pages/groups.detail.ts
Spec: First check docs/product/ for a dedicated spec file for F-17. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-17.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/groups.detail.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/groups.detail.ts
...
Assert every import in src/pages/groups.detail.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-groups-detail.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-groups-detail.test.ts
git commit -m "test(gk): F-17 group entry requirements — gk tests"
git push
