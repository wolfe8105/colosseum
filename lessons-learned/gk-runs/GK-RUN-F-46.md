# GATEKEEPER RUN — F-46 Private Lobby
...
Feature: F-46 — Private Lobby
Source file: src/arena/arena-private-lobby.ts
Spec: First check docs/product/ for a dedicated spec file for F-46. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-46.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-private-lobby.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-private-lobby.ts
...
Assert every import in src/arena/arena-private-lobby.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-private-lobby.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-private-lobby.test.ts
git commit -m "test(gk): F-46 private lobby — gk tests"
git push
