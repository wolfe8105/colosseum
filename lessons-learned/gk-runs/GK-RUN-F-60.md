# GATEKEEPER RUN — F-60 Saved Loadouts
...
Feature: F-60 — Saved Loadouts
Source file: src/arena/arena-loadout-presets.ts
Spec: First check docs/product/ for a dedicated spec file for F-60. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-60.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/arena/arena-loadout-presets.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/arena/arena-loadout-presets.ts
...
Assert every import in src/arena/arena-loadout-presets.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-arena-loadout-presets.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-arena-loadout-presets.test.ts
git commit -m "test(gk): F-60 saved loadouts — gk tests"
git push
