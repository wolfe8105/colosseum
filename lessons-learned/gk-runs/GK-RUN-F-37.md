# GATEKEEPER RUN — F-37 Notification Preferences
...
Feature: F-37 — Notification Preferences
Source file: src/pages/settings.ts
Spec: First check docs/product/ for a dedicated spec file for F-37. If none, use docs/THE-MODERATOR-PUNCH-LIST.md row F-37.
...
FILE READ PROOF: For every file you read, state filename and exact line count before acting on it. Format: READ: src/pages/settings.ts — N lines

STEP 0 — DEPENDENCY MAP
Read _archive/prototypes/colosseum-critical-path-v3.txt. If missing, query codebase directly.
Report:
MAP QUERY — src/pages/settings.ts
...
Assert every import in src/pages/settings.ts is on the allowed list from Step 2.

STEP 5 — RUN AND REPORT
npm test -- tests/gk-settings.test.ts
Also run regression surface tests.

If PASS:
git add tests/gk-settings.test.ts
git commit -m "test(gk): F-37 notification preferences — gk tests"
git push
