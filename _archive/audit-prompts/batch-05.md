...
pre-filled for Batch 5.

---

```
Run audit Batch 5 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 5 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 5,
if an old manifest from Batch 4 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch04.json, then create the fresh manifest
for Batch 5.
...
Do not advance to Batch 6 on your own. Stop after Batch 5
and wait.
```
