...
pre-filled for Batch 12.

---

```
Run audit Batch 12 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 12 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 12,
if an old manifest from Batch 11 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch11.json, then create the fresh manifest
for Batch 12.
...
This is the final batch. After completion, report the full run
summary across all 12 batches if possible, then STOP.
```
