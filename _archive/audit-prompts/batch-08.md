...
pre-filled for Batch 8.

---

```
Run audit Batch 8 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 8 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 8,
if an old manifest from Batch 7 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch07.json, then create the fresh manifest
for Batch 8.
...
Do not advance to Batch 9 on your own. Stop after Batch 8
and wait.
```
