...
2. FIRST-RUN-FILES.md
   The file list. For this run, use Batch F only. Ignore the other
   batches. The file list is under the "## Batch F" heading.
...
audit/manifest.batchPREV.json, then create the fresh manifest for Batch F.
...
Do not advance to the next batch on your own. Stop after Batch F and wait.
```
