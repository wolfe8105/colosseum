...
pre-filled for Batch 4.

---

```
Run audit Batch 4 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 4 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 4,
if an old manifest from Batch 3 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch03.json, then create the fresh manifest
for Batch 4.
...
Do not advance to Batch 5 on your own. Stop after Batch 4
and wait.
```
