...
2. FIRST-RUN-FILES.md
   The file list. For this run, use Batch C only. Ignore the other
   batches. The file list is under the "## Batch C" heading.
...
audit/manifest.batchPREV.json, then create the fresh manifest for Batch C.
...
Do not advance to the next batch on your own. Stop after Batch C and wait.
```
