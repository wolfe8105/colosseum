...
pre-filled for Batch 11.

---

```
Run audit Batch 11 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 11 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 11,
if an old manifest from Batch 10 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch10.json, then create the fresh manifest
for Batch 11.
...
Do not advance to Batch 12 on your own. Stop after Batch 11
and wait.
```
