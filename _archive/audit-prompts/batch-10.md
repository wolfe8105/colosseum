...
pre-filled for Batch 10.

---

```
Run audit Batch 10 from THE MODERATOR repo.
...
2. FIRST-RUN-FILES.md
   ...
   use Batch 10 only. Ignore the other batches.
...
Each batch run gets its own manifest. Before starting Batch 10,
if an old manifest from Batch 9 (or earlier) is present at
audit/manifest.json, archive it by renaming to
audit/manifest.batch09.json, then create the fresh manifest
for Batch 10.
...
Do not advance to Batch 11 on your own. Stop after Batch 10
and wait.
```
