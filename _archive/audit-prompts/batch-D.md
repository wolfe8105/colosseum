...
2. FIRST-RUN-FILES.md
   The file list. For this run, use Batch D only. Ignore the other
   batches. The file list is under the "## Batch D" heading.
...
audit/manifest.batchPREV.json, then create the fresh manifest for Batch D.
...
Do not advance to the next batch on your own. Stop after Batch D and wait.
```
