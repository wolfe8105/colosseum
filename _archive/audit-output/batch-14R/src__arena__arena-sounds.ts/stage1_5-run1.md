...
- `_ctx` (line 14): excluded — `let` binding to a null value, not a function definition.
- `SOUNDS` (line 136): excluded — `const` binding to an object literal (`Record<...>`), not a function definition.
- `SoundName` (line 146): excluded — exported type alias, not a callable binding.
- `IntroTrack` (lines 170–175): excluded — exported interface, not a callable binding.
- `INTRO_TRACKS` (line 177): excluded — `const` binding to an array literal, not a function definition.
- `INTRO_SYNTHS` (line 303): excluded — `const` binding to an object literal (`Record<...>`).
- `_introAudioEl` (line 316): excluded — `let` binding to a null value, not a function definition.
- Inline callbacks inside track synthesizers: excluded — inner callbacks, not top-level named bindings.
