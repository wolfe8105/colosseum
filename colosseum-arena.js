...
window.ColosseumArena = (() => {
  ...
  // SESSION 121: State-driven popstate handler. No skip flags, no DOM checks.
  // Forward navigation: overlays use replaceState (no history.back, no async race).
  // Backward navigation (hardware back / cancel): popstate always returns to lobby.
  ...
  function closeRankedPicker(forward) {
    const overlay = document.getElementById('arena-rank-overlay');
    if (overlay) {
      overlay.remove();
      if (forward) {
        // Moving forward in flow — replace overlay entry with lobby, no async back()
        history.replaceState({ arenaView: 'lobby' }, '');
      } else {
        // Cancel / backdrop / hardware back — navigate back in history
        history.back();
      }
    }
  }
  ...
  function closeModeSelect(forward) {
  ...
  function enterRoom(debate) {
    ...
    // SESSION 121: Set debate status to 'live' now that user has entered
    // (create_ai_debate inserts as 'pending' so staking works on pre-debate screen)
    if (debate.mode === 'ai' && !isPlaceholder() && !debate.id.startsWith('ai-local-') && !debate.id.startsWith('placeholder-')) {
      safeRpc('update_arena_debate', { p_debate_id: debate.id, p_status: 'live' }).catch(e => {
    ...
    // SESSION 121: Staking panel removed from debate room — handled by showPreDebate screen
