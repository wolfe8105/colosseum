  ...
  function showRulingPanel(ref) {
    ...
    overlay.querySelector('#mod-ruling-allow')?.addEventListener('click', async () => {
      ...
      if (typeof ColosseumAuth !== 'undefined' && ColosseumAuth.ruleOnReference) {
        ...
        if (result?.error) {
          addSystemMessage('❌ Ruling failed: ' + result.error);
        } else {
  ...
  function startReferencePoll(debateId) {
    ...
    referencePollTimer = setInterval(async () => {
      ...
      if (debate?.moderatorId && debate.moderatorId === profile.id) {
        refs.filter(r => r.status === 'pending' && !seenRefs.has(r.id)).forEach(ref => {
          seenRefs.add(ref.id);
          showRulingPanel(ref);
        });
      }
      ...
      refs.filter(r => (r.status === 'allowed' || r.status === 'denied') && !seenRefs.has(r.id + '-ruled')).forEach(ref => {
        seenRefs.add(ref.id + '-ruled');
        const icon = ref.status === 'allowed' ? '✅' : '❌';
        addSystemMessage(`${icon} Evidence ${ref.status.toUpperCase()}${ref.ruling_reason ? ': ' + ref.ruling_reason : ''}`);
      });
    }, 3000);
  }
    ...
    section.querySelectorAll('.mod-score-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (typeof ColosseumAuth !== 'undefined' && ColosseumAuth.scoreModerator) {
          ...
          if (result?.error) {
            if (scoredEl) { scoredEl.textContent = '❌ ' + result.error; scoredEl.style.display = 'block'; scoredEl.style.color = 'var(--red)'; }
          } else {
