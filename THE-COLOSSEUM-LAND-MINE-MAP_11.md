...
STATUS: FIXED Session 48. stripe_processed_events table + INSERT ON CONFLICT.
RISK: FIXED.
SESSION: 45 (discovered), 48 (fixed).
```
...
STATUS: FIXED Session 48. rawBody = await req.text() confirmed in rewrite.
RISK: FIXED.
SESSION: 45 (discovered), 48 (fixed).
```
