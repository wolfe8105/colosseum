...
- **F-24 Search — FULLY SPECCED S253.** Build dependency: F-23 must ship first (bidirectional block hide references `dm_blocks`). 1 new table (`search_index`) + 1 new column (`profiles.searchable`) + 3 RPCs + 3 triggers + 1 cron. `pg_trgm` GIN index, engagement-boosted nightly cron refresh, users/debates/groups at launch (references and hot takes deferred). Users can opt out of user-search via `searchable = false` column (minimal opt-out, not full private profile — F-54 is the full-profile toggle). One-CC-session job after F-23. Full spec in pending spec file. Land Mines: LM-220/221/222/223.
