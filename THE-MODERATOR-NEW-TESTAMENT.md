...
### Last Updated: Session 275 (April 12, 2026)
...
| Modifier / Power-Up system (scoring effects) | **F-57 — SHIPPED S267 (phases 1-4) + S269 deferred cluster.** 6 new tables, 12 new RPCs, `src/modifiers.ts`. 50 of 59 effects live. 9 effects deferred (architectural blockers). See F-57 critical notes section below. |
...
- **Session 271: All remaining dependencies wired — F-51 is code-complete.** WebRTC audio bridge (`joinDebate`/`leaveDebate` in `arena-feed-room.ts`). `deepgram-token` Edge Function deployed (key was already live in Supabase secrets). AdSense publisher ID `ca-pub-1800696416995461` + ad unit `8647716209` wired into `showAdOverlay()`. **Pending: live end-to-end test (two debaters + moderator).**
