...
### Deferred Effects (9) — architectural blockers
| Effect(s) | Blocker |
|---|---|
| `pressure`, `momentum` | Need round-end trigger / per-round score history |
| `mirror`, `burn_notice`, `parasite`, `chain_reaction` | Opponent inventory manipulation — separate session |
| `streak_saver` (token earn path), `token_drain` (out-of-debate library challenges), + 1 misc | Minor gaps deferred |
