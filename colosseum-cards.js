...
// SESSION 65 SECURITY AUDIT (Session 15): 2 bugs fixed
//   - BUG 1 (LOW): Download filename uses unvalidated opts.size
//   - BUG 2 (LOW): No input length limits on side labels
// ============================================================
...
window.ColosseumCards = (() => {
  ...
  function generateCard(opts) {
    // BUG 1 FIX: Validate size against allowlist
    const sizeKey = VALID_SIZES.has(opts.size) ? opts.size : 'og';
    ...
    // BUG 2 FIX: Truncate side labels
  ...
  function downloadCard(opts) {
    ...
    // BUG 1 FIX: Use validated size key in filename, not raw input
    const sizeKey = VALID_SIZES.has(opts.size) ? opts.size : 'og';
