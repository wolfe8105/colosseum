...
### Created: Session 27 (March 2, 2026) | Scenarios 1A/1B/1C updated: Session 56 (March 8, 2026) — three-zone architecture
...
> **Architecture note (updated Session 56):** The platform uses a three-zone model.
> - **Zone 1 — Static Mirror** (`colosseum-f30.pages.dev`): Pure HTML, zero JS, zero auth. Generated every 5 minutes by the VPS mirror generator, deployed to Cloudflare Pages. All bot links point here. This is what strangers see first.
> - **Zone 2 — Plinko Gate** (`colosseum-six.vercel.app/colosseum-plinko.html`): Signup/login flow. 4 steps: OAuth → Age gate → Username → Done. The bridge from stranger to member.
> - **Zone 3 — Members App** (`colosseum-six.vercel.app`): Full Vercel app. Auth required for write actions. Guest mode for read-only browsing.
>
> Scenarios 1A/1B previously described users landing directly on the Vercel app. That was wrong. Bot links always go to the mirror. The mirror has no interactive features — it's bait. Actions that require auth redirect to Zone 3 (Vercel), which shows auth prompts.

## 1A: User taps a Bluesky rage-bait link to an auto-debate verdict

1. User sees bot post on Bluesky with a provocative debate verdict and a link
2. User taps link
3. Mobile browser opens `colosseum-f30.pages.dev/debate/[slug]` — the static mirror page
4. Mirror page loads instantly (pure HTML, no JS, no auth, served from Cloudflare CDN)
5. Page displays: debate topic, both sides' arguments (summarized), AI-scored winner, lopsided score
6. No login wall. No popup. No cookie banner. Nothing blocks reading.
7. User sees the "wrong" winner — rage reaction kicks in
8. Page shows a CTA button: "Disagree? Join the Colosseum and vote." (static link, no JS)
9. User taps CTA → redirected to `colosseum-six.vercel.app/colosseum-debate-landing.html?id=XXXX`
10. Debate landing page loads in Vercel app (Zone 3) — now dynamic
11. Full debate content visible
12. "Vote" button visible
13. User taps vote
14. Auth prompt appears: "Sign up to vote" — Google/Apple OAuth prominent, email behind toggle
15. If user dismisses → stays on landing page, can keep reading, vote not recorded
16. If user signs up → Plinko Gate (Scenario 2), then returns to this landing page
17. After signup, vote is recorded
18. Vote count updates visually
19. User can navigate to home, categories, arena from here

**Edge cases:**
- Mirror link is stale (debate was deleted after mirror last generated) → mirror page still shows cached content. If user clicks through to Vercel landing page, that page shows "Debate not found."
- Mirror itself is down (Cloudflare outage) → nothing loads. Bot links go dead. Vercel app is unaffected.
- Mirror regeneration delayed (VPS hiccup) → mirror shows content up to 5 minutes old. Acceptable.
- User taps link on slow mobile connection → Cloudflare CDN serves static HTML fast regardless. No JS to block render.
- User is on a browser without JavaScript → mirror renders fine (pure HTML). Vercel app may degrade gracefully.

## 1B: User taps a Bluesky link to a category hot take

1. User sees bot post on Bluesky referencing a specific hot take or debate topic with a link
2. User taps link
3. Mobile browser opens `colosseum-f30.pages.dev/category/[slug]` — static mirror category page
4. Mirror page loads: shows the category name, recent bot-seeded hot takes (static HTML list), and debate summaries
5. No interactive elements on this page — it's a read-only snapshot
6. Page has a CTA: "See live takes and join the debate" (link to Vercel app)
7. User taps CTA → redirected to `colosseum-six.vercel.app/?screen=category&cat=[slug]`
8. Vercel app loads (Zone 3), spoke carousel appears, category overlay opens automatically
9. Hot takes tab shows live hot takes (dynamic, real data from Supabase)
10. Predictions tab shows active prediction questions
11. User can scroll both tabs freely
12. User taps a hot take reaction button → auth prompt appears
13. User taps "Post a take" → auth prompt appears
14. User taps a username → profile modal opens (no auth needed to view)
15. User can navigate back to home, other categories, arena — all without auth

**Edge cases:**
- Category mirror page has zero content (bots haven't posted to this category yet) → mirror shows "No debates yet in this category" message, CTA still present
- Category slug in URL is invalid → mirror returns its 404 page. Vercel app URL fallback: invalid `cat=` param → app defaults to spoke carousel home view.
- Mirror page CTA link takes user to Vercel app, but Vercel is slow → loading state visible (auth init running), spoke carousel renders within 4 seconds max (auth timeout fires)

## 1C: User visits the platform directly (no bot link)

> This scenario has two distinct sub-cases depending on which URL the user types or finds.

### 1C-i: User finds the mirror URL (colosseum-f30.pages.dev)

1. User types or bookmarks `colosseum-f30.pages.dev`
2. Mirror home page loads instantly — pure HTML, static
3. Shows: platform description, list of recent debate categories with hot take counts, links to debate pages
4. No interactivity. No auth. No spoke carousel. Just a read-only index.
5. CTA visible: "Enter the Colosseum" → links to `colosseum-six.vercel.app`
6. User taps CTA → Zone 3 (Vercel app)

### 1C-ii: User finds the Vercel app URL (colosseum-six.vercel.app)

1. User types `colosseum-six.vercel.app` directly (e.g., from a reference or prior visit)
2. Brief loading screen appears (auth init running — INITIAL_SESSION + 5s safety timeout)
3. Auth resolves as guest (no session cookie) within ~1 second typically, 5 seconds worst case
4. Spoke carousel loads with 6 categories visible
5. Activity indicators visible on tiles (live debate count, hot take count, or "Quiet" if empty)
6. User can spin/tap categories — everything readable, nothing gated
7. User taps a category → overlay opens with live hot takes and predictions
8. User can navigate to arena, leaderboard, etc. from navigation
9. All read-only browsing works without auth

**Edge cases (both sub-cases):**
- First visit ever, no cookies, no session → guest mode immediately, everything works
- User previously had a session, cookies cleared → guest mode, auth prompt on any write action
- Mirror is the only thing that shows up in Google search (SEO intent) — this is by design. Mirror is Google-indexed. Vercel app is the destination after engagement.
- User bookmarks the Vercel app URL directly → works fine, loads as guest every time until they log in
...
|---|----------|--------|-----------|-------|
| 1A | Guest: Bluesky link → mirror verdict → Vercel vote | ⬜ | | Updated Session 56 |
| 1B | Guest: Bluesky link → mirror category → Vercel app | ⬜ | | Updated Session 56 |
| 1C-i | Direct visit to mirror (colosseum-f30.pages.dev) | ⬜ | | Added Session 56 |
| 1C-ii | Direct visit to Vercel app (colosseum-six.vercel.app) | ⬜ | | Updated Session 56 |
