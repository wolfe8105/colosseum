...
**Spec fully locked Session 278. Build-ready.**

**Scope at launch:** Singles only (player vs player). Groups deferred. **Format:** Single-elimination. Round-robin second once single-elim is stable. Double-elim and Swiss deferred until demand justifies. **Debate format:** Standard ranked — no live moderator required in the room. F-51 moderated tournament tier deferred: single-sitting format + small early mod community = staffing risk at launch. Revisit once mod base is large enough to staff a full bracket in one day. **ELO/XP impact:** Full — tournament debates affect ranked ELO and XP exactly like regular ranked debates. No special casing.

**Seeding:** ELO-based. Gives ELO real stakes beyond the ranked ladder. **Bracket size:** Min 8, max 64, dynamic fill. No fixed power-of-2 requirement. **Bye assignment:** Highest ELO gets the bye. "It's good to be the king." **Scheduling:** Single-sitting only at launch — creator sets a start time, all rounds run that day. Multi-day deferred. **Bracket lock:** Auto-locks at the creator's chosen start time. Late entries not allowed after lock. **Drop-out / no-show:** Auto-forfeit after 10 minutes. No ELO or token penalty.

**Creation gate:** F-33 Verified Gladiator badge (60% profile depth). Hard gate, nothing stacked on top. This is the launch gate, not a future upgrade. **Entry fees:** Min 10 tokens, max 1000 tokens at launch (raised later as trust grows). **Escrow:** Entry fees go into platform escrow at join, never to the creator. Creators cannot rug-pull. **Cancellation:** Creator can cancel before bracket lock — full refund to all entrants, no creator penalty. No cancellation after lock. **Refunds:** Unfilled bracket slots (below min 8) refunded automatically at lock time if tournament fails to fill.

**Prize distribution (singles):** Top-3 split. Winners pool = 90% when no mod present, 85% when mod present. Platform takes 10%. Mod pool = 5% only when a moderator is assigned to the tournament — split evenly per match moderated. Within the winners pool: 1st takes 70%, 2nd takes 20%, 3rd takes 10%.

**Tournament-only power-ups:** Cut from v1. Can be added later once bracket system is running.

**Spectator betting:** Deferred.

**UI / discovery:** Tournaments surface in the existing category debate feed — no new nav destination. When a player's bracket match is generated, the gold fast-blink dot appears on the top-left corner of the bell icon (distinct from the magenta notification pulse on the top-right). Tapping the bell takes the player to notifications where the match card is surfaced. The debate itself runs in the standard arena room with a tournament UI layer on top: gold tournament banner (tournament name + prize pool), bracket position strip (QF → SF → Final, you-are-here marker), match context (round, countdown, what a win/loss means in tokens). Match card in the feed is tagged as tournament — players and spectators can identify it as such.

**Bracket engine (core infrastructure):** A `tournaments` table stores tournament metadata. A `tournament_matches` table stores bracket pairings with round number, match slot, and outcome. When a match completes, `arena_debates.tournament_match_id` foreign key reports the result back to the bracket. The bracket engine advances winners and generates next-round pairings automatically. `arena_debates` needs a `tournament_match_id` column. Tournament debates pay reference royalties at full rate from the normal token faucet — royalties do not come out of the prize pool.

**Moderator compensation cosmetics (future):** When F-51 tournament tier ships, tournament-exclusive cosmetics unlock at moderation milestones (Tournament Moderator badge, exclusive borders, tournament-only titles) via the F-31 auto-unlock trigger system already shipped.
