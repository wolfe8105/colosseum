...
### 1a. Picker dismiss handlers (backdrop click and cancel button)

Each picker in the configuration chain has both a backdrop tap and a cancel/back button that dismiss the overlay. All dismiss handlers remove the overlay from DOM and call `history.back()` to return to the previous picker (or lobby if at the first picker). Four picker pairs are covered: mode select, category, ranked, and ruleset.

<!-- captured: src/arena/arena-config-mode.ts:115 -->
<!-- captured: src/arena/arena-config-mode.ts:116 -->
<!-- captured: src/arena/arena-config-mode.ts:255 -->
<!-- captured: src/arena/arena-config-mode.ts:261 -->
<!-- captured: src/arena/arena-config-settings.ts:91 -->
<!-- captured: src/arena/arena-config-settings.ts:92 -->
<!-- captured: src/arena/arena-config-settings.ts:161 -->
<!-- captured: src/arena/arena-config-settings.ts:162 -->

```mermaid
sequenceDiagram
    actor User
    participant ModeBackdrop as arena-mode-backdrop click<br/>arena-config-mode.ts:115
    participant ModeCancel as arena-mode-cancel click<br/>arena-config-mode.ts:116
    participant CloseMFn as closeModeSelect()<br/>arena-config-mode.ts:119
    participant CatCancel as arena-cat-cancel click<br/>arena-config-mode.ts:255
    participant CatBackdrop as arena-cat-backdrop click<br/>arena-config-mode.ts:261
    participant RankBackdrop as arena-rank-backdrop click<br/>arena-config-settings.ts:91
    participant RankCancel as arena-rank-cancel click<br/>arena-config-settings.ts:92
    participant CloseRkFn as closeRankedPicker()<br/>arena-config-settings.ts:95
    participant RuleBackdrop as arena-ruleset-backdrop click<br/>arena-config-settings.ts:161
    participant RuleCancel as arena-ruleset-cancel click<br/>arena-config-settings.ts:162
    participant CloseRsFn as closeRulesetPicker()<br/>arena-config-settings.ts:165

    alt Mode picker dismiss (backdrop)
        User->>ModeBackdrop: tap mode backdrop
        ModeBackdrop->>CloseMFn: closeModeSelect()
    else Mode picker dismiss (cancel)
        User->>ModeCancel: tap Cancel
        ModeCancel->>CloseMFn: closeModeSelect()
    end
    CloseMFn->>CloseMFn: overlay.remove() then history.back()

    alt Category picker dismiss (backdrop)
        User->>CatBackdrop: tap category backdrop
        CatBackdrop->>CatBackdrop: overlay.remove() then history.back()
    else Category picker dismiss (cancel)
        User->>CatCancel: tap Back button
        CatCancel->>CatCancel: overlay.remove() then history.back()
    end

    alt Ranked picker dismiss (backdrop)
        User->>RankBackdrop: tap ranked backdrop
        RankBackdrop->>CloseRkFn: closeRankedPicker()
    else Ranked picker dismiss (cancel)
        User->>RankCancel: tap Cancel
        RankCancel->>CloseRkFn: closeRankedPicker()
    end
    CloseRkFn->>CloseRkFn: overlay.remove() then history.back()

    alt Ruleset picker dismiss (backdrop)
        User->>RuleBackdrop: tap ruleset backdrop
        RuleBackdrop->>CloseRsFn: closeRulesetPicker()
    else Ruleset picker dismiss (cancel)
        User->>RuleCancel: tap Cancel
        RuleCancel->>CloseRsFn: closeRulesetPicker()
    end
    CloseRsFn->>CloseRsFn: overlay.remove() then history.back()
```

**Notes:**
- captured: src/arena/arena-config-mode.ts:115 — `#arena-mode-backdrop` click fires `closeModeSelect()`.
- captured: src/arena/arena-config-mode.ts:116 — `#arena-mode-cancel` click fires `closeModeSelect()`.
- captured: src/arena/arena-config-mode.ts:255 — `#arena-cat-cancel` click inline-removes overlay and calls `history.back()`.
- captured: src/arena/arena-config-mode.ts:261 — `#arena-cat-backdrop` click inline-removes overlay and calls `history.back()`.
- captured: src/arena/arena-config-settings.ts:91 — `#arena-rank-backdrop` click fires `closeRankedPicker()`.
- captured: src/arena/arena-config-settings.ts:92 — `#arena-rank-cancel` click fires `closeRankedPicker()`.
- captured: src/arena/arena-config-settings.ts:161 — `#arena-ruleset-backdrop` click fires `closeRulesetPicker()`.
- captured: src/arena/arena-config-settings.ts:162 — `#arena-ruleset-cancel` click fires `closeRulesetPicker()`.
- Mode picker dismiss: `closeModeSelect()` at `arena-config-mode.ts:119` removes the overlay. When the `forward` parameter is omitted (falsy), it calls `history.back()` instead of `history.replaceState()`.
- Category picker dismiss: the cancel button and backdrop both inline-remove the overlay and call `history.back()` directly (no shared close function).
- Ranked and ruleset picker dismissals follow the same pattern as mode — their close functions check the `forward` parameter.
- All dismiss handlers navigate backward through the browser history stack pushed by `pushArenaState()` in the forward flow.
...
sequenceDiagram
    ...
    MatchFn->>StatusEl: "OPPONENT FOUND!"<br/>(gold color accent)
...
sequenceDiagram
    ...
    StatusFn->>User: status text changes to<br/>"Queue still active - searching..."
    ...
    alt User taps SPAR WITH AI
        ...
        PromptUI->>EnterAI: enterQueue(ai, "")
...
- captured: src/arena/arena-queue.ts:158 — the `#arena-queue-ai-spar` button click handler at line 158 is the specific interactive element for the SPAR WITH AI button. The above diagram covers this flow via the "User taps SPAR WITH AI" alt path. The handler calls `leaveQueue()` then `enterQueue('ai', '')`.
...
sequenceDiagram
    ...
    alt User taps SPAR WITH AI
        User->>OptionsUI: tap
        OptionsUI->>User: enterQueue(ai, "")
    else User taps TRY AGAIN
        User->>OptionsUI: tap
        OptionsUI->>User: enterQueue(selectedMode, "")
