...
| F-05 | Debate recording + replay | | ✅ Pending spec | 🔶 Session 233: Replay enrichment live (power-ups, references, mod scores displayed in spectate via new `get_debate_replay_data` RPC). Session 246: F-05 fully walked and locked in pending spec file. (1) AI scorecard persistence — SQL already live (`session-234-ai-scorecard-persistence.sql`: `ai_scorecard` JSONB column + `save_ai_scorecard` RPC + `get_arena_debate_spectator` updated), only remaining work is client-side write in `src/arena/arena-room-end.ts` after AI scoring completes. (2) Inline point awards — unblocked by F-57 modifier system + F-51 §6.6 edit. Format `+N × M = T pts` on comment line, archive stores base/modifier/final as separate columns. See pending spec file for full paragraph. |
...
| F-58 | Sentiment tipping (paid gauge movement) | | ✅ Pending spec (S249) | New feature walked S249. Replaces scratched F-51 Phase 5 free-vote model — no free vote button, every point of gauge movement costs tokens. 2/3/5/10 tip-button strip, min 2 tokens, unlimited cumulative taps. Tier gate based on NEW `debate_watches` table (Observer 1 / Fan 5 / Analyst 15 / Insider 50). Binary unlock — no per-tier amount caps. 50% refund on winning side, burn on losing side, burn on draws. Live + human-vs-human only (no AI sparring, no replays). Both-side tipping allowed, no side-lock. Cap 1 billion per side per debate. 2 new tables (`debate_watches`, `sentiment_tips`), 2 new columns on `arena_debates`, 4 new RPCs (`log_debate_watch`, `get_user_watch_tier`, `cast_sentiment_tip`, `settle_sentiment_tips`), drops `cast_sentiment_vote` RPC. Client wiring in `arena-feed-room.ts`, `arena-types.ts`, `spectate.ts`. Dwell data explicitly OUT of scope — S249 research confirmed dwell only monetizes when ad inventory exists, no near-term ad plans. Full spec in pending spec file. |
