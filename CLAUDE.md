...
**Backend:** Supabase project `faomczmipsccwbhpivmp`. 34 tables, 42+ server functions, RLS hardened.
...
- **`safeRpc()`** wraps all RPC calls with error handling (not yet backfilled everywhere)
...
- `THE-COLOSSEUM-NEW-TESTAMENT.md` — condensed project knowledge, key decisions, current state
- `THE-COLOSSEUM-OLD-TESTAMENT.md` — all session build logs (1-62), 502+ item inventory
- `THE-COLOSSEUM-LAND-MINE-MAP.md` — 165+ documented pitfalls and fixes. **Read before any SQL, schema, auth, or deployment change.**
