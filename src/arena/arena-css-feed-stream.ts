...
export function injectFeedStreamCSS(): void {
  ...
  style.textContent = `
    ...
    .feed-evt-a { background:rgba(231,68,42,0.08);border-left:3px solid #E7442A; /* TODO: needs CSS var token */ align-self:flex-start; }
    .feed-evt-b { background:rgba(74,144,217,0.08);border-left:3px solid #4A90D9; /* TODO: needs CSS var token */ align-self:flex-start; }
    .feed-evt-mod { background:rgba(194,154,88,0.10);border-left:3px solid #c29a58; /* TODO: needs CSS var token */ align-self:flex-start; }
    .feed-evt-name { font-size:11px;font-weight:700;letter-spacing:1px;display:block;margin-bottom:2px; }
    .feed-evt-a .feed-evt-name { color:#E7442A; /* TODO: needs CSS var token */ }
    .feed-evt-b .feed-evt-name { color:#4A90D9; /* TODO: needs CSS var token */ }
    .feed-evt-mod .feed-evt-name { color:#c29a58; /* TODO: needs CSS var token */ }
    .feed-evt-text { color:var(--mod-text-primary); }
    .feed-evt-points { text-align:center;align-self:center;max-width:100%; }
    .feed-points-badge { display:inline-block;padding:4px 12px;border-radius:var(--mod-radius-pill);background:rgba(194,154,88,0.15);border:1px solid rgba(194,154,88,0.3);color:#c29a58; /* TODO: needs CSS var token */ font-family:var(--mod-font-ui);font-size:12px;font-weight:700;letter-spacing:1px; }
