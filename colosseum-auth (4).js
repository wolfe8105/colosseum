...
window.ColosseumAuth = (() => {
  ...
  function init() {
    ...
    try {
      ...
      // SESSION 27: Kill navigator.locks before Supabase can use it.
      // Orphaned locks cause getSession() to hang forever, which blocks
      // every .rpc() call. Supabase falls back to a tab-local storage
      // approach. Trade-off: no cross-tab session sync (irrelevant for mobile).
      try {
