    ...
    FROM (
      ...
      (
        ...
        SELECT aud.id, aud.topic, 'auto_debate'::text as source, 'ai'::text as mode, aud.status,
               ...
               aud.final_score_a as score_a, aud.final_score_b as score_b,
               (SELECT count(*)::int FROM auto_debate_votes WHERE auto_debate_id = aud.id AND vote = 'a') as vote_count_a,
               (SELECT count(*)::int FROM auto_debate_votes WHERE auto_debate_id = aud.id AND vote = 'b') as vote_count_b,
               aud.side_a_name as debater_a_name, 0 as elo_a,
               aud.side_b_name as debater_b_name, 0 as elo_b,
               aud.created_at
