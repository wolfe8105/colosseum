...
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZhb21jem1pcHNjY3diaHBpdm1wIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIxOTM4NzIsImV4cCI6MjA4Nzc2OTg3Mn0.d11AoWVu074DHo3vjVNNOA-1DT8KaoAXF341ysLoHYI';
const SIGNUP_URL = process.env.SIGNUP_URL || 'https://colosseum-six.vercel.app/colosseum-plinko.html';
const MIRROR_DOMAIN = process.env.MIRROR_DOMAIN || 'https://colosseum.pages.dev';
...
async function fetchAutoDebates() {
  return supaQuery('auto_debates', 'status=eq.active&order=created_at.desc&limit=50&select=id,topic,side_a_label,side_b_label,winner,score_a,score_b,category,rounds,created_at,verdict');
}
...
async function fetchLeaderboard() {
  return supaQuery('profiles', 'order=elo_rating.desc&limit=25&select=display_name,username,elo_rating,wins,losses,level,debates_completed');
}
...
<div class="footer">
  ...
  <a href="https://colosseum-six.vercel.app/colosseum-terms.html">Terms</a>
</div>
</body>
</html>`;
}
...
<div class="card debate-card">
  ...
  <div class="card-meta">${d.category ? esc(d.category) + ' · ' : ''}${timeAgo(d.created_at)}</div>
</div>
</a>`;
}
...
<div class="card" style="border-color:rgba(212,168,67,0.3)">
  ...
  ${d.verdict ? `<div class="card-text" style="margin-top:10px">${esc(d.verdict)}</div>` : ''}
</div>`;
...
function deploy() {
  ...
  try {
    ...
    execSync(`npx wrangler pages deploy ${OUT_DIR} --project-name=${CLOUDFLARE_PROJECT} --branch=main`, {
      stdio: 'inherit',
      timeout: 60000,
    });
