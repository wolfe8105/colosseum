...
window.ColosseumAuth = (() => {
  ...
  function init() {
    ...
    try {
      ...
      // SESSION 31: Pass noOpLock directly into createClient.
      // Supabase v2 auth uses navigator.locks internally and deadlocks
      // when orphaned locks exist (GitHub issue supabase-js#1594).
      // The global navigator.locks mock (colosseum-locks-fix.js) doesn't
      // work because the auth client uses its own internal lock method.
      // This constructor option bypasses it at the source.
