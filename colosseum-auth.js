...
window.ColosseumAuth = (() => {
  ...
  function init() {
    ...
    try {
      ...
      // SESSION 27/30: Replace navigator.locks with a no-op mock.
      // Orphaned locks cause getSession(), signOut(), and token refresh to hang.
      // Setting locks=undefined (Session 27) broke signOut and session persistence
      // because Supabase's fallback code path is buggy.
      // Fix: mock that instantly resolves — Supabase stays on its normal code path
      // but locks never actually block. Trade-off: no cross-tab session sync (irrelevant for mobile).
      try {
        Object.defineProperty(navigator, 'locks', {
          value: {
            request: async function(_name, optionsOrCb, maybeCb) {
              // Web Locks API has two forms:
      ...
      } catch (e) { /* older browsers may not have it anyway */ }
  ...
  async function _checkSession() {
    ...
    // SESSION 26: Race getSession against a 3s timeout.
    // Supabase auth can hang on orphaned navigator locks — never block the app.
    let resolved = false;
    const safeResolve = () => { if (!resolved) { resolved = true; _resolveReady(); } };

    // Timeout: resolve ready even if getSession never returns
    const timeout = setTimeout(() => {
      console.warn('ColosseumAuth: getSession timed out after 3s — continuing as guest');
      safeResolve();
    }, 3000);
  ...
  async function logOut() {
    ...
    try {
      // SESSION 30: Timeout wrapper — signOut can hang if locks mock fails.
