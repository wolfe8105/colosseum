...
// ============================================================
// THE COLOSSEUM — AI GENERATOR
// Uses Groq (free tier, Llama 3.1 70B) to generate:
...
// - Full AI vs AI debate rounds (Leg 3 — Session 20)
// - Lopsided scoring + rage-bait hooks (Leg 3 — Session 20)
...
// SESSION 75: Strip source attribution from RSS headlines
// "Cowboys trade DT Osa... - NBC Sports" → "Cowboys trade DT Osa..."
// "Nicole Kidman Says... - People?" → "Nicole Kidman Says..."
function cleanHeadline(headline) {
  if (!headline) return '';
  let h = headline.trim();
  // Strip trailing ? that got appended to source
  if (h.endsWith('?') && h.includes(' - ')) {
    const lastDash = h.lastIndexOf(' - ');
    const afterDash = h.substring(lastDash + 3).replace(/\?$/, '').trim();
    // If the part after " - " has no spaces or is short (likely a source name), strip it
...
// ============================================================
// SYSTEM PROMPTS — personality of the bot
// ============================================================
...
{"title": "New immigration bill: progress or failure?", "sideA": "Real progress", "sideB": "Not enough", "description": "Congress passed a bipartisan immigration bill. Does it actually solve anything?"}`,
  ...
  // ============================================================
  // SESSION 20 — AUTO-DEBATE PROMPTS (Leg 3)
  // ============================================================
...
// ============================================================
// CORE GENERATION FUNCTIONS (Leg 1 + 2 — unchanged)
// ============================================================

async function generateHotTake(headline, category = 'general') {
  headline = cleanHeadline(headline); // SESSION 75: strip source attribution
  try {
    ...
    logger.error(`Hot take generation failed: ${err.message}`);
    return fallbackHotTake(headline);
  }
}

async function generateReply(originalText, platform = 'reddit') {
  try {
    const client = getClient();
    ...
    return fallbackReply(originalText);
  }
}

async function generateDebateTopic(headline) {
  headline = cleanHeadline(headline); // SESSION 75: strip source attribution
  try {
    const client = getClient();
    const response = await client.chat.completions.create({
    ...
    logger.error(`Debate topic generation failed: ${err.message}`);
    return fallbackDebateTopic(headline);
  }
}

// ============================================================
// LEG 3 — AUTO-DEBATE GENERATION (Session 20)
// ============================================================

async function generateAutoDebateSetup(headline, category = 'general') {
  headline = cleanHeadline(headline); // SESSION 75: strip source attribution
  try {
    const client = getClient();
    const response = await client.chat.completions.create({
    ...
    logger.error(`Auto-debate setup generation failed: ${err.message}`);
    return fallbackAutoDebateSetup(headline);
  }
}

async function generateAutoDebateRound(setup, roundNum, totalRounds, previousRounds, winningSide) {
  try {
    const client = getClient();
    ...
    logger.error(`Auto-debate round ${roundNum} generation failed: ${err.message}`);
    return fallbackAutoDebateRound(setup, roundNum);
  }
}

async function generateAutoDebateScore(setup, rounds, winningSide, margin) {
  try {
    const client = getClient();
    ...
    logger.error(`Auto-debate scoring failed: ${err.message}`);
    return fallbackAutoDebateScore(rounds.length, winningSide, margin);
  }
}

async function generateShareHook(setup, winningSide, margin) {
  try {
    const client = getClient();
    ...
    logger.error(`Share hook generation failed: ${err.message}`);
...
function fallbackHotTake(headline) {
  // SESSION 95: All templates include headline, no repetitive openers
...
function fallbackReply(originalText) {
  const templates = [
...
function fallbackDebateTopic(headline) {
  return {
...
function fallbackAutoDebateSetup(headline) {
  // SESSION 75: Clean source attribution, generate topic-aware labels
  const clean = cleanHeadline(headline);
  const topic = clean.length > 95 ? clean.substring(0, 92) + '...' : clean;
  // Try to generate meaningful side labels from headline context
...
function fallbackAutoDebateRound(setup, roundNum) {
  // SESSION 93: 5 openers x 5 middles x 5 closers per side = 125 combos each
...
function fallbackAutoDebateScore(totalRounds, winningSide, margin) {
  const scores = [];
  let totalA = 0;
  let totalB = 0;
...
function fallbackShareHook(setup, winningSide) {
