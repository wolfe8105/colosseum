...
window.ColosseumArena = (() => {
  ...
  window.addEventListener('popstate', (e) => {
    ...
    if (view === 'postDebate') {
      ...
      // Already at lobby — let browser handle normally
      // (go back to whatever page was before arena)
    }
  });
    ...
    overlay.querySelectorAll('.arena-rank-card').forEach(card => {
      card.addEventListener('click', async () => {
        ...
        if (isRanked && !isPlaceholder()) {
          try {
            const sb = getSupabase();
            const { data, error } = await sb.rpc('check_ranked_eligible');
  ...
  async function endCurrentDebate() {
    ...
    if (!isPlaceholder() && !debate.id.startsWith('ai-local-') && !debate.id.startsWith('placeholder-')) {
      try {
        const { data: result, error } = await getSupabase()?.rpc('update_arena_debate', {
          p_debate_id: debate.id,
          p_status: 'complete',
          p_winner: winner,
          p_score_a: scoreA,
