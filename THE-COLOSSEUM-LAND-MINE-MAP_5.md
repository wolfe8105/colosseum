...
### Last Updated: Session 45 (March 7, 2026)
...
IMPACT: Users who navigate directly to login page may experience hang.
  The navigator.locks fix (Session 31) is verified working on index.html but
  login.html has not been separately tested. May have its own hang condition.
...
*Land Mine Map v1 — Session 29. Updated Session 42: LM-120 through LM-123 added.*
...
## LM-121: Groq llama-3.1-70b-versatile is dead
```
LESSON (Session 42): Groq decommissioned llama-3.1-70b-versatile.
  API returns 400 with "model_decommissioned" error code.
  Bot army was falling back to template responses for all AI generation.
SYMPTOM: Groq 400 errors in pm2 logs. "[AI] Using fallback template" messages.
  Auto-debates still created but with generic canned text.
FIX: Updated bot-config.js on VPS to llama-3.3-70b-versatile.
  ⚠️ Edge Functions (ai-sparring, ai-moderator) still reference the
  old model — they will fail when called. Need redeployment via
  Supabase CLI on VPS with updated model string.
SESSION: 42.
```

---

## LM-122: VPS git divergent branches — stash+rebase+pop
```
LESSON (Session 42): VPS had a local commit (mirror generator fix)
  that couldn't push (no GitHub auth token). When git pull ran,
  it failed with "divergent branches" error.
SYMPTOM: git pull fails with "Need to reconcile divergent branches"
FIX: git stash && git pull origin main --rebase && git stash pop
  This saves local changes, rebases onto remote, then restores.
  If you're okay losing local changes: git reset --hard origin/main
SESSION: 42.
```
...
## LM-090: Mirror generator anon key was wrong by 1 character
```
LESSON (Session 33): Key had '341' instead of '340' in the JWT signature.
  Curl with the wrong key returned "Invalid API key" — but the error was
  a single character making the whole token invalid.
RULE: When Supabase returns "Invalid API key", do a character-by-character
  diff against the dashboard key. Don't assume the whole key is wrong.
SESSION: 33.
```
...
## LM-106: index.html is the mirror output — NOT the app shell
```
DECISION (Session 38): Mirror generator writes index.html to _mirror_build/.
  Someone uploaded the mirror output to the repo root, overwriting the real app shell.
PROTECTS: N/A — this was an accident, not a decision.
BITES YOU WHEN: You deploy index.html from repo to Vercel — logged-in users
  see a static page with zero interactivity, no JS modules, no bottom nav.
SYMPTOM: App loads but nothing works. No spoke carousel, no arena, no leaderboard.
  Just a static marketing page.
FIX: Real app shell is 546+ lines with 5 screen containers, 16 JS imports,
  bottom nav bar. Recoverable from git commit e8b7b3b. Session 38 restored it.
RULE: Never name mirror output index.html in the repo root. Mirror generator
  builds to _mirror_build/ — that's where it belongs. Cloudflare deploys from there.
SESSION: 38. See also LM-091 (mirror queries must match live schema).
```

---

## LM-107: Login redirect must await ColosseumAuth.ready
```
DECISION (Session 37): Login page checks if user is already logged in and redirects.
PROTECTS: Logged-in users skip the login page.
BITES YOU WHEN: You check ColosseumAuth.currentUser synchronously at
  DOMContentLoaded. INITIAL_SESSION hasn't fired yet — currentUser is null.
  Logged-in user sees the login form and nothing redirects them.
SYMPTOM: Login page appears to "hang" for logged-in users.
FIX: Await ColosseumAuth.ready.then(() => { check redirect }).
  The readyPromise resolves after INITIAL_SESSION fires.
  Never check auth state synchronously at page load.
SESSION: 37.
```

---

## LM-108: vercel.json route destinations must match actual filenames
```
LESSON (Session 38): /privacy route pointed to colosseum-terms.html
  instead of colosseum-privacy.html. Wrong page served for months.
RULE: After adding any new HTML page, verify vercel.json routes.
  Grep for the filename in vercel.json to confirm it's wired correctly.
SESSION: 38.
```

---

## LM-109: Leaderboard time filters need schema support
```
DECISION (Session 37): Leaderboard UI has week/month/all-time buttons.
  All three currently show the same data because profiles_public only
  has cumulative stats (elo_rating, wins, losses, current_streak).
PROTECTS: N/A — this is a known limitation, not a bug.
BITES YOU WHEN: Users click "THIS WEEK" and see the same rankings
  as "ALL TIME." Looks broken even though it's by design.
FIX: When time-bucketed stats are added to schema (e.g. weekly_wins,
  monthly_elo_change), wire the filter into _fetchLeaderboard().
  Until then, all filters show cumulative stats.
SESSION: 37.
```
...
## LM-111: get_debate_references() — return type change needs DROP FUNCTION
```
LESSON (Session 39): Live Supabase had an existing get_debate_references()
  with a different return type. CREATE OR REPLACE cannot change return types.
  Same pattern as LM-095 (auto_allow_expired_references).
SYMPTOM: "cannot change return type of existing function"
FIX: Run DROP FUNCTION get_debate_references(uuid) first,
  then CREATE OR REPLACE. This applies to ANY function where
  the return type changed between versions.
NOTE: Column on debate_references is 'ruling', NOT 'status'.
  Query it as r.ruling = 'pending'|'allowed'|'denied'.
SESSION: 39.
```
...
## LM-115: deploy-verify.sh expects wrong filename
```
LESSON (Session 40): deploy-verify.sh on VPS checks for
  colosseum-settings_3.html but the actual repo file is
  colosseum-settings.html. Also false-positive on password regex
  matching <input type="password"> in colosseum-plinko.html.
SYMPTOM: DEPLOY BLOCKED — 2 critical failures that are both false positives.
FIX: Update deploy-verify.sh:
  1. Change settings_3 → settings in the file check
  2. Exclude HTML input fields from secret pattern regex
  Location: /opt/colosseum/deploy-verify.sh on VPS
SESSION: 40.
```

---

## LM-116: Edge Function paths changed on VPS
```
DECISION (Session 40): Edge Functions now live at
  supabase/functions/{name}/index.ts on VPS (standard Supabase CLI path).
PROTECTS: Supabase CLI deploy works correctly.
BITES YOU WHEN: You look for Edge Function source at old paths:
  - colosseum-ai-sparring/ (DELETED)
  - ai-moderator-index.ts (DELETED)
  These don't exist anymore. Don't recreate them.
FIX: Edge Function source is at:
  /opt/colosseum/supabase/functions/ai-sparring/index.ts
  /opt/colosseum/supabase/functions/ai-moderator/index.ts
  Deploy: supabase functions deploy {name} --project-ref faomczmipsccwbhpivmp
SESSION: 40.
```

---

## LM-117: Stripe Edge Functions still use old import pattern
```
DECISION (Session 40): Stripe Edge Function templates in
  colosseum-stripe-functions.js still use deno.land/std and esm.sh imports.
  NOT YET REWRITTEN with Deno.serve + npm: pattern.
PROTECTS: N/A — this is tech debt, not a decision.
...
FIX: When redeploying Stripe functions, apply same pattern as ai-sparring:
  Deno.serve() instead of serve(), npm: imports instead of esm.sh/deno.land.
  Not urgent — deployed version is frozen and working.
SESSION: 40.
```

---

## LM-118: Cloudflare Pages project name vs URL
```
DECISION (Session 41): Pages project is named "colosseum" but the URL
  is colosseum-f30.pages.dev (not colosseum.pages.dev — name was taken).
PROTECTS: N/A — Cloudflare assigned the suffix.
BITES YOU WHEN: You use colosseum.pages.dev in config, .env, bot army
  links, or documentation. That URL doesn't exist.
SYMPTOM: 404 or DNS error on colosseum.pages.dev.
FIX: Always use colosseum-f30.pages.dev. Bot army .env already set.
  When thecolosseum.app domain is purchased, point it to Pages project
  via Cloudflare custom domain — then both URLs work.
  wrangler command uses --project-name=colosseum (the project name, not URL).
SESSION: 41.
```

---

## LM-119: Cloudflare API token must be rotated
```
LESSON (Session 41): API token was displayed in chat log during setup.
PROTECTS: N/A — this is a hygiene issue.
BITES YOU WHEN: Chat logs are compromised. Token gives Edit access to
  Workers, Pages, KV, R2, and all Cloudflare resources.
SYMPTOM: Unauthorized deploys to Pages, Workers created/modified.
FIX: Rotate at dash.cloudflare.com/profile/api-tokens when convenient.
  Delete old token, create new one, update VPS .env with new value.
  Not urgent but do it before any public launch.
SESSION: 41.
```

---

# SECTION 17: SESSION 43 FIXES
...
## LM-125: deploy-verify.sh scans too many index.ts files
```
LESSON (Session 43): Original EF_FILES find command searched entire
  repo for "index.ts" — picked up 30+ files from node_modules.
  Also: grep -c returns "0\n0" instead of "0" for some files,
  breaking integer comparison in SRI check.
BITES YOU WHEN: You run deploy-verify.sh and see 30 CORS check lines
  or "integer expression expected" errors.
FIX: EF scan must be scoped to supabase/functions/ directory only.
  SRI check must assign grep result with || fallback pattern:
  cdn_scripts=$(grep -c "pattern" "$f" 2>/dev/null) || cdn_scripts=0
SESSION: 43.
```
...
## LM-130: Stripe webhook has no idempotency — will double-fire on retries
```
DECISION (built Session ~20): Stripe Edge Function processes every
  incoming webhook event without checking if it was already processed.
PROTECTS: Nothing — this is a gap.
BITES YOU WHEN: Stripe retries a webhook (network hiccup, 5xx, their
  own retry logic up to 3 days). Same event fires twice.
  Result: user gets double-credited tokens, double-subscribed,
  or double-fulfilled. This is a money bug.
SYMPTOM: Users with 2x token balance. Duplicate subscription rows.
  No error in logs — everything succeeds, just twice.
FIX: Add stripe_processed_events table (id TEXT PRIMARY KEY).
  Before processing: INSERT ... ON CONFLICT DO NOTHING RETURNING id.
  If no row returned → already processed → return 200 immediately.
  Combined fix with LM-131 (raw body) in same rewrite session.
RISK: HIGH once Stripe goes live. Zero risk in sandbox.
SESSION: 45 (discovered in security audit).
```

---

## LM-131: Stripe webhook may not preserve raw body for HMAC verification
```
DECISION (built Session ~20): Unknown if Edge Function reads body
  with req.text() (correct) or req.json() then re-stringifies (wrong).
PROTECTS: Nothing — this is a gap.
BITES YOU WHEN: If body is parsed then re-stringified, whitespace
  differences silently break HMAC verification. Every webhook fails
  with signature mismatch. Stripe retries for 3 days then gives up.
SYMPTOM: All webhooks fail silently. No token credits. No subscription
  activations. Stripe dashboard shows repeated delivery failures.
FIX: Read raw body with `const rawBody = await req.text()`.
  Pass rawBody directly to stripe.webhooks.constructEvent().
  Do NOT parse then re-stringify.
COMBINED FIX: Fix LM-130 + LM-131 in same session (same file).
  Also rewrite with Deno.serve() pattern (LM-117).
RISK: HIGH once Stripe goes live.
SESSION: 45 (discovered in security audit).
```

---

## LM-132: Rate limiting race condition under concurrent load
```
DECISION (built Session 16-17): rate_limits table in Postgres.
  Per-action checks inside RPCs: read count → compare → increment.
  No atomic lock on the read-compare-increment sequence.
PROTECTS: Supposed to prevent spam/abuse.
BITES YOU WHEN: Two concurrent requests hit the same RPC at the same
  time. Both read "count = 4, limit = 5". Both pass. Both increment.
  Count is now 6 but both slipped through at 4.
  Under real traffic (bot army driving clicks), this happens constantly.
SYMPTOM: Rate limits appear to work but don't hold under load.
  Abuse possible at moderate concurrent traffic.
FIX (Phase 1, low effort): Add FOR UPDATE to rate_limits read:
  SELECT * FROM rate_limits WHERE ... FOR UPDATE.
  This row-locks and serializes concurrent checks.
FIX (Phase 2, post-traction): Migrate to Upstash Redis sliding window.
  Return X-RateLimit-Remaining and Retry-After headers.
RISK: MEDIUM. Doesn't collapse the app. Allows some abuse under load.
SESSION: 45 (discovered in security audit).
```

---

## LM-133: RLS policies likely missing indexes and role scoping
```
DECISION (built Session 14-17): 24 RLS policies built for security.
  Index status and role scoping unknown.
PROTECTS: Nothing new — this is a performance and correctness gap.
BITES YOU WHEN: Tables grow. auth.uid() called per-row without index.
  Leaderboard, hot takes feed, arena lobby all slow down. 100x+ slower
  on large tables without btree index on user_id columns.
  Also: policies without TO authenticated evaluate for anon users,
  wasting DB cycles on requests that will always return empty.
SYMPTOM: Slow queries. Supabase Performance Advisor will flag them.
FIX: Run Supabase Dashboard → Database → Performance Advisor.
  Also Security Advisor. Both auto-detect missing indexes + role gaps.
  Add (SELECT auth.uid()) wrapper to all auth.uid() calls in policies.
  Add TO authenticated to all policies scoped to logged-in users.
  Add btree indexes on user_id columns in high-traffic tables.
RISK: LOW now, MEDIUM at scale.
SESSION: 45 (discovered in security audit).
```
...
## LM-135: debit_tokens() had no caller guard
```
DECISION (built Session 16): debit_tokens() is SECURITY DEFINER and
  accepts p_user_id UUID as a parameter. No check on who the caller is.
  Any authenticated user could call debit_tokens(victim_uuid, 9999)
  and drain another user's token balance.
PROTECTS: Nothing — this was a gap.
BITES YOU WHEN: A user discovers they can call the RPC directly via
  supabase.rpc('debit_tokens', { p_user_id: '...', p_amount: 9999 }).
  Token ledger is drained silently. No audit trail on who called it.
SYMPTOM: Users complain their balance disappears. token_transactions
  shows debits with reason 'spend' from unknown triggers.
FIX: Add service_role guard matching credit_tokens:
  v_caller := current_setting('request.jwt.claim.role', true);
  IF v_caller IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'debit_tokens: service_role only';
  END IF;
STATUS: FIXED Session 46. SQL confirmed in Supabase.
RISK: HIGH (was). FIXED.
SESSION: 46.
```
...
## LM-137: Cloudflare API token was in chat log (Session 41)
```
DECISION: Cloudflare API token was pasted/visible in a prior session
  chat log (Session 41, during wrangler setup).
BITES YOU WHEN: Anyone with access to the conversation history can
  extract the token and deploy arbitrary content to colosseum-f30.pages.dev,
  or make API calls under the account.
FIX: Roll the token at dash.cloudflare.com/profile/api-tokens.
  Update /opt/colosseum/mirror.env on VPS with new token.
  Run pm2 restart all.
STATUS: FIXED Session 46. Token rolled, VPS updated.
SESSION: 46 (fix). Original exposure: Session 41.
```
