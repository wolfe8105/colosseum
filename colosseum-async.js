...
window.ColosseumAsync = (() => {
  ...
  let hotTakes = [];
  let predictions = [];
  let standaloneQuestions = []; // SESSION 113: Bug 026
  ...
  function _wireTakeDelegation(container) {
    container.addEventListener('click', (e) => {
      ...
      if (action === 'react') react(btn.dataset.id);
        ...
        if (!card) return;
        const textEl = card.querySelector('[data-action="expand"]');
        if (textEl) { textEl.style.display = ''; textEl.style.webkitLineClamp = 'unset'; textEl.style.overflow = 'visible'; }
        const moreEl = card.querySelectorAll('[data-action="expand"]')[1];
  ...
  function _wirePredictionDelegation(container) {
    container.addEventListener('click', (e) => {
      ...
      // SESSION 113: Standalone prediction picks
      else if (btn.dataset.action === 'standalone-pick') {
        pickStandaloneQuestion(btn.dataset.id, btn.dataset.pick);
      }
      // SESSION 113: Open create form
  ...
  // SESSION 113: Added create button and standalone questions
  ...
  // ============================================================
  // SESSION 113: STANDALONE PREDICTION QUESTIONS — Bug 026
  // ============================================================

  // --- Fetch standalone questions from Supabase ---
  async function fetchStandaloneQuestions(category) {
    if (!ColosseumAuth?.supabase || ColosseumAuth.isPlaceholderMode) return;
    try {
      const { data, error } = await ColosseumAuth.safeRpc('get_prediction_questions', {
        p_limit: 20,
        p_category: category || null
      });
      if (error) throw error;
      if (data && Array.isArray(data) && data.length > 0) {
        standaloneQuestions = data;
      }
    } catch (e) {
  ...
  function _renderStandaloneCard(q) {
    ...
    const userPick = q._userPick || null; // set locally after picking
  ...
  // --- Pick on a standalone question ---
  async function pickStandaloneQuestion(questionId, side) {
    ...
    if (!q) return;
    if (q._userPick === side) return; // already picked
    ...
    if (ColosseumAuth?.supabase && !ColosseumAuth.isPlaceholderMode) {
      try {
        const { data, error } = await ColosseumAuth.safeRpc('pick_prediction', {
  ...
  // --- Create Prediction Form (bottom sheet) ---
  function openCreatePredictionForm() {
    ...
    document.getElementById('cpq-submit').addEventListener('click', async () => {
      ...
      const sideA = document.getElementById('cpq-side-a').value.trim() || 'Yes';
      const sideB = document.getElementById('cpq-side-b').value.trim() || 'No';
      const category = document.getElementById('cpq-category').value || null;

      if (topic.length < 10) {
        if (ColosseumConfig?.showToast) ColosseumConfig.showToast('Question must be at least 10 characters', 'error');
        return;
      }
      ...
      if (ColosseumAuth?.supabase && !ColosseumAuth.isPlaceholderMode) {
        try {
          const { data, error } = await ColosseumAuth.safeRpc('create_prediction_question', {
            p_topic: topic,
          ...
          if (ColosseumConfig?.showToast) ColosseumConfig.showToast('Failed to create prediction', 'error');
      ...
      standaloneQuestions.unshift({
        ...
        picks_b: 0,
        total_picks: 0,
        creator_display_name: ColosseumAuth?.currentProfile?.display_name || 'You',
        _userPick: null,
      });

      overlay.remove();
      if (ColosseumConfig?.showToast) ColosseumConfig.showToast('🔮 Prediction posted!', 'success');
  ...
  return {
    ...
    fetchStandaloneQuestions, // SESSION 113
